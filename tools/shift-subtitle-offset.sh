...
# Manually shift subtitle tracks by a user-specified offset (ms).
# Positive offset = subtitles appear later; negative = earlier.
...
j = json.loads(info)
tracks = [t for t in j.get("tracks", []) if t.get("type") == "subtitles"]
for t in tracks:
    tid = t.get("id")
...
prompt_file() {
  ...
  while true; do
    read -rp "$prompt: " f
    [[ -n "$f" ]] || { echo "Please enter a file path."; continue; }
    [[ -f "$f" ]] && echo "$f" && return
...
echo "Subtitle tracks in: ${files[0]}"
list_sub_tracks "${files[0]}"

if [[ -n "${TRACK_IDS:-}" ]]; then
  track_ids="$TRACK_IDS"
else
  read -rp "Track IDs to shift (comma-separated, blank = all subtitle tracks): " track_ids
fi
...
apply_sync_flags() {
  ...
  if [[ -z "$track_ids" ]]; then
    mapfile -t subs < <("$PYTHON_BIN" - <<'PY' "$f"
import json, subprocess, sys
f = sys.argv[1]
j = json.loads(subprocess.check_output(["mkvmerge", "-J", f]))
ids = [str(t["id"]) for t in j["tracks"] if t["type"] == "subtitles"]
print("\n".join(ids))
PY
)
  else
    IFS=',' read -r -a subs <<<"$track_ids"
  fi

  for sid in "${subs[@]}"; do
    sid="${sid//[[:space:]]/}"
    [[ -n "$sid" ]] && _flags+=(--sync "${sid}:${offset_ms}")
  done
}
