...
REPO_URL="${REPO_URL:-https://github.com/nekotrix/SVT-AV1-Essential.git}"
BRANCH="${BRANCH:-Essential-v4.0.1}"
# Use /config/encoders if it exists (Docker), else fall back to project dir
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -d "/config/encoders" ]]; then
  DEFAULT_PREFIX="/config/encoders/essential"
else
  DEFAULT_PREFIX="$SCRIPT_DIR/config/encoders/essential"
fi
...
Options:
  ...
  --build PATH         Build dir (default: <src>/build-essential)
  --jobs N             Parallel jobs (default: $JOBS)
  --update [pull|reset]
                       pull  = git pull (may conflict on rebases)
                       reset = git fetch && git reset --hard origin/<branch> (default)
...
if [[ "$USE_TEMP_WORKDIR" -eq 1 ]]; then
  TEMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/essential-build.XXXXXX")"
  ...
  if [[ -z "$SRC_DIR" ]]; then
    SRC_DIR="$HOME/src/svt-av1-essential"
  fi
  if [[ -z "$BUILD_DIR" ]]; then
    BUILD_DIR="$SRC_DIR/build-essential"
  fi
fi
...
echo "Wrapper: $PREFIX/bin/essential"
if [[ "$USE_TEMP_WORKDIR" -eq 1 ]]; then
