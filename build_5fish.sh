...
REPO_URL="${REPO_URL:-https://github.com/5fish/svt-av1-psy.git}"
BRANCH="${BRANCH:-exp}"
# Use /config/encoders if it exists (Docker), else fall back to project dir
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -d "/config/encoders" ]]; then
  DEFAULT_PREFIX="/config/encoders/5fish"
else
  DEFAULT_PREFIX="$SCRIPT_DIR/config/encoders/5fish"
fi
...
Options:
  ...
  --build PATH         Build dir (default: <src>/build-5fish)
...
if [[ "$USE_TEMP_WORKDIR" -eq 1 ]]; then
  TEMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/5fish-build.XXXXXX")"
  ...
  if [[ -z "$SRC_DIR" ]]; then
    SRC_DIR="$HOME/src/svt-av1-psy-5fish"
  fi
  if [[ -z "$BUILD_DIR" ]]; then
    BUILD_DIR="$SRC_DIR/build-5fish"
  fi
fi
